...
// Figma border: rgba(155,126,189,0.1) — subtle purple tint
const BORDER        = 'rgba(155,126,189,0.1)';
...
// ─── MetricCard (standard) ────────────────────────────────────────────────────
//
// Layout (matches Figma MetricCard.desktop — node 311:3250):
//   flex-col gap-8 · padding 16.5px · border-radius 10px
//   ┌ icon (20×20)
//   └ metric.figures (flex-col gap-6)
//       ┌ Value  (h:20, fixed — prevents null/default height difference)
//       └ metric.name (justify-between: label left · tooltip right)
...
export function MetricCard({
  ...
  // null: 16px (em dash); mobile default: 14px; desktop default: 20px
  // The Value wrapper has fixed height 20px, so font-size doesn't affect card height
  const valueFontSize = isNull ? 16 : isMobile ? 14 : 20;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%' }}>
        ...
        {/* Value — fixed 20px height prevents null vs default height difference */}
        ...
        {/* metric.name row: label + tooltip icon */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: isNull ? 'flex-start' : 'space-between',
          gap: isNull ? 12 : 0,
          width: '100%',
        }}>
          <span style={{
