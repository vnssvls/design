...
const DESKTOP_BORDER = 'rgba(155,126,189,0.1)';
const MOBILE_BORDER  = 'rgba(255,255,255,0.1)';
...
function IconCOGS({ color, size = 20 }: { color: string; size?: number }) {
  return (
...
function IconWastage({ color, size = 20 }: { color: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 28 28" fill="none">
...
function IconUndefinedSales({ color, size = 20 }: { color: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 28 28" fill="none">
...
function IconVariance({ color, size = 20 }: { color: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 28 28" fill="none">
...
function MetricIcon({ type, state, size = 20 }: { type: MCType; state: MCState; size?: number }) {
  const color = getIconColor(type, state);
  switch (type) {
    case 'cogs':            return <IconCOGS color={color} size={size} />;
    case 'wastage':         return <IconWastage color={color} size={size} />;
    case 'undefined-sales': return <IconUndefinedSales color={color} size={size} />;
    case 'variance':        return <IconVariance color={color} size={size} />;
  }
}
...
function TooltipIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <rect x="6.5" y="6" width="1" height="4.5" rx="0.5" fill="rgba(255,255,255,0.45)" />
      <circle cx="7" cy="4.25" r="0.75" fill="rgba(255,255,255,0.45)" />
    </svg>
  );
}
...
// ─── MetricCard (standard) ────────────────────────────────────────────────────
//
// Desktop layout (Figma node 311:3250):
//   flex-col · gap 8 · padding 16.5px · w 161px · border-radius 10
//   icon 20×20 · value 20px · label 12px · tooltip icon desktop only
//
// Mobile layout (Figma node — mobile):
//   flex-row · height 61px fixed · padding 12.5px 16.5px · w 161px
//   icon 15×15 · gap 12 between icon and figures · value 16px · label 12px
//   border rgba(255,255,255,0.1) · no tooltip
//
// Null state (both):
//   em dash · value rgba(255,255,255,0.88) · bg TONAL_NULL_BG (tonal theme)
...
export function MetricCard({
  ...
  const bg = theme === 'grey'
    ? GREY_BG
    : isNull ? TONAL_NULL_BG : TONAL_BG;

  const border  = isMobile ? MOBILE_BORDER : DESKTOP_BORDER;
  const iconSz  = isMobile ? 15 : 20;
  const valueFz = isMobile ? 16 : isNull ? 16 : 20;

  const displayValue = isNull ? '—' : (value ?? '12.50%');
  ...
  if (isMobile) {
    return (
      <div style={{
        ...
        {/* Icon — 15×15 */}
        <div style={{ width: iconSz, height: iconSz, flexShrink: 0 }}>
          <MetricIcon type={type} state={state} size={iconSz} />
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 20, height: 20, flexShrink: 0 }}>
        <MetricIcon type={type} state={state} size={20} />
      </div>

      {/* metric.figures */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%' }}>
        {/* Value — fixed 20px height prevents null/default height shift */}
        ...
        {/* metric.name: label left · tooltip right (non-null only) */}
        <div style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: (!isNull && showTooltip) ? 'space-between' : 'flex-start',
          gap: 12,
          width: '100%',
        }}>
          <span style={{
...
//
// Layout (Figma node 1643:2286):
//   Three cards in a flex-row · gap 8
//   Each card: flex-row · align-center · padding 16px H / 12px V
//   Left: 32×32 icon container (flex-shrink 0)
//   Right: flex-col gap-4 · value 16px 600 · label 11px CACACA
